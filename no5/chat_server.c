...
#define MAX_MESSAGE_LENGTH (M_LINE + 20)
    ...
    while (1) {
        ...
        for (i = 0; i < n_user; i++) {
            if (FD_ISSET(client_sock_list[i], &read_fds)) {
                ...
                int user_id_len = strlen(user_ids[i]);
                int msg_len = strlen(buf);
                
                // 전체 메시지 길이가 MAX_MESSAGE_LENGTH를 초과하지 않도록 조정
                if (user_id_len + msg_len + 2 > MAX_MESSAGE_LENGTH) { // +2 for ": "
                    msg_len = MAX_MESSAGE_LENGTH - user_id_len - 2;
                }

                snprintf(message, sizeof(message), "%s: %.*s", user_ids[i], msg_len, buf);
